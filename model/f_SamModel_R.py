...
class SoftModel():
	...
	def __init__(self,obj_feature_no_align, actual_batch,relation_feature,video_feature, 
    input_captions, dec_x_t, dec_h_tm1, v_encoder_output, r_encoder_output, voc_size, d_w2v, output_dim, 
		...
		#print('SamModel_VRel_GRU.py: __init__: obj_feature:', obj_feature.get_shape().as_list())
		...
		print('SamModel_VRel_GRU.py: __init__: video_feature:', video_feature.get_shape().as_list())
		...
		#fyt
		#print('SamModel_VRel_GRU.py: __init__: video_feature:', obj_feature_no_align_feature.get_shape().as_list())
		self.obj_feature_no_align = tf.transpose(obj_feature_no_align,perm=[1,0,2,4,5,3]) #  [obj_num, batch, timestep, h, w, c]
		self.actual_batch = actual_batch
		self.relation_feature = relation_feature ## relation graph [batch, timestep, obj_num, obj_num]
		...
		self.r_encoder_output = r_encoder_output
		...
		self.enc_in_shape_obj = self.obj_feature_no_align.get_shape().as_list() # [obj_num, batch, timestep, h, w, c(2048)]
		self.decoder_input_shape = self.input_captions.get_shape().as_list() # [batch, caplen]
		#print('SamModel_VRel_GRU.py: __init__: self.enc_in_shape_video:', self.enc_in_shape_video,
	...
	def init_parameters(self):
		...
		#fyt
		############## relation encoder parameters for GRU
		relation_encoder_shape = (self.reduction_dim, self.reduction_dim)
		'''
		self.W_11 = tf.get_variable("W_11",relation_encoder_shape,initializer=tf.random_normal_initializer(stddev=1./math.sqrt(self.reduction_dim)))
		self.W_21 = tf.get_variable("W_21",relation_encoder_shape,initializer=tf.random_normal_initializer(stddev=1./math.sqrt(self.reduction_dim)))
		self.W_12 = tf.get_variable("W_12",relation_encoder_shape,initializer=tf.random_normal_initializer(stddev=1./math.sqrt(self.reduction_dim)))
		self.W_22 = tf.get_variable("W_22",relation_encoder_shape,initializer=tf.random_normal_initializer(stddev=1./math.sqrt(self.reduction_dim)))
		self.W_13 = tf.get_variable("W_13",relation_encoder_shape,initializer=tf.random_normal_initializer(stddev=1./math.sqrt(self.reduction_dim)))
		self.W_23 = tf.get_variable("W_23",relation_encoder_shape,initializer=tf.random_normal_initializer(stddev=1./math.sqrt(self.reduction_dim)))
		
		self.B1 = tf.get_variable("B1",initializer = tf.random_normal([self.reduction_dim], stddev=1./math.sqrt(self.reduction_dim)))
		self.B2 = tf.get_variable("B2",initializer = tf.random_normal([self.reduction_dim], stddev=1./math.sqrt(self.reduction_dim)))
		self.B3 = tf.get_variable("B3",initializer = tf.random_normal([self.reduction_dim], stddev=1./math.sqrt(self.reduction_dim)))
		'''
		## relation encoder parameters for GCN
		self.w1_re = tf.get_variable("w1_re",relation_encoder_shape,initializer=tf.random_normal_initializer(stddev=1./math.sqrt(self.reduction_dim)))
		self.w2_re = tf.get_variable("w2_re",relation_encoder_shape,initializer=tf.random_normal_initializer(stddev=1./math.sqrt(self.reduction_dim)))
		...
		###### attention parameters for relation feature fyt
		self.W_r_a = tf.get_variable("W_r_a",[self.reduction_dim,self.attention_dim],
			initializer=tf.random_normal_initializer(stddev=1./math.sqrt(self.reduction_dim)))

		self.U_r_a = tf.get_variable("U_r_a",[self.output_dim,self.attention_dim],initializer=tf.random_normal_initializer(stddev=1./math.sqrt(self.output_dim)))
		self.b_r_a = tf.get_variable("b_r_a",initializer = tf.random_normal([self.attention_dim],stddev=1. / math.sqrt(self.attention_dim))) 

		self.W_r = tf.get_variable("W_r",(self.attention_dim,1),initializer=tf.random_normal_initializer(stddev=1. / math.sqrt(self.attention_dim)))
		self.r_A = tf.get_variable("r_A",(self.reduction_dim,3*self.output_dim),
			initializer=tf.random_normal_initializer(stddev=1./ math.sqrt(self.reduction_dim)))
	...
	def init_embedding_matrix(self):
		...
		#print('SamModel_VRel_GRU.py: init_embedding_matrix starts ...')
		voc_size = self.voc_size # 12597
		d_w2v = self.d_w2v	# 512
		print('SamModel_VRel_GRU.py: init_embedding_matrix: voc_size=', voc_size, 'd_w2v=', d_w2v)
		...
		print('SamModel_VRel_GRU.py: init_embedding_matrix: T_mask:',T_mask.get_shape().as_list())
		...
		print('SamModel_VRel_GRU.py: init_embedding_matrix: T_w2v:',T_w2v.get_shape().as_list())
		#print('SamModel_VRel_GRU.py: init_embedding_matrix ends ...')
		return T_w2v, T_mask 
	...
	def encoder(self, input_feature): ### for video feature
		...
		print('SamModel_VRel_GRU.py: encoder: assignment:', assignment)
		axis = [1,0]+list(range(2,5)) 
	...
	#fyt
  #input_feature: obj_feature_no_align shape[obj_num, batch, timestep, h, w, c]  
	#timestep-40 h-7 w-7 c-2048
	#output:relation_encoder_output shape[batch, timestep, redu_dim] 
	#output is pool of the hidden state of the GRU
	#G: relation graph [batch,timestep,obj_num,obj_num]
	def encoder_relation(self, input_feature, G):
		print('#####encoder_relation#######')
		#print(self.timesteps)
		#print(self.batch_size_v)
		#print(input_feature)
		#print(G)
    ...
    # to be continued gcn
		...
		#input_feature = tf.reshape(input_feature,[-1,self.obj_num,self.timesteps,self.vfmap_h,self.vfmap_w,self.reduction_dim])
		#assert (1==2)

		input_feature = tf.reshape(input_feature,[self.obj_num,-1,self.timesteps,self.vfmap_h,self.vfmap_w,self.reduction_dim])
		input_feature = tf.reduce_mean(input_feature, axis=[3,4])#[obj,batch,timestep,redu]
		#input_feature = tf.reduce_mean(input_feature, 3) 
		#print(input_feature)
		#assert(1==2)
		input_feature = tf.transpose(input_feature,[2,1,0,3]) #[timestep, batch, obj, redu]

		G = tf.transpose(G,[1,0,2,3]) #[timestep,batch,obj,obj]
		#correct!!
		#print(input_feature)
		#print(G)
		#assert(1==2)
		...
		#assert(1==2)
		# GRU
		#input array gcn_output [timestep, batch, redu_dim]
		#output: array [timestep, batch, redu_dim]
		'''
		def GRU(rela_feature):

			relation_timesteps = self.timesteps

			#timesteps * [batch, redu_dim]
			relation_hidden_state = tf.TensorArray(
				dtype = tf.float32,
				size = relation_timesteps,
				tensor_array_name = 'relation_hidden_states'
				)

			def relation_step(x_t, h_tm1):

				r = hard_sigmoid(tf.matmul(x_t, self.W_11)+tf.matmul(h_tm1, self.W_21)+self.B1)
				z = hard_sigmoid(tf.matmul(x_t, self.W_12)+tf.matmul(h_tm1, self.W_22)+self.B2)
				hh = tf.nn.tanh(tf.matmul(x_t, self.W_13)+tf.matmul(r*h_tm1, self.W_23)+self.B3)
			
				h = (1-z)*hh + z*h_tm1
				return h

			def relation_train_step(time, relation_hidden_state, h_tm1 ):
			 
				x_t = rela_feature[time]  #shape [batch, redu_dim]

				h = relation_step(x_t, h_tm1) #shape [batch,redu_dim]
				
				relation_hidden_state = relation_hidden_state.write(time,h)

				return(time+1, relation_hidden_state, h)


			initial_state = tf.zeros([self.actual_batch,self.reduction_dim])#!!need to be improved       huge problem!!
			time = tf.constant(0, dtype='int32', name='time')
			relation_train_out = tf.while_loop(
				cond = lambda time,*_: time < relation_timesteps,
				body = relation_train_step,
				loop_vars = (time, relation_hidden_state, initial_state),
				parallel_iterations = 32,
				swap_memory = True
				)

			relation_hidden_state = relation_train_out[1] #timesteps * [batch, redu_dim]
			
			if hasattr(relation_hidden_state, 'stack'):
				gru_outputs = relation_hidden_state.stack()
			else:
				gru_outputs = relation_hidden_state.pack() # array [timesteps, batch, redu_dim]
		...
		#GCN model 
		#G[timestep, batch, obj_num, obj_num] relation graph
		#!!!!need to be improved
		#print('########### here!! ########')
		#print(self.actual_batch)
		#print(self.timesteps*self.actual_batch)
		#assert(1==2)
		#print(G[1,2])
		#print(G[1][2])
		#assert(1==0)
		...
		def GCN(o_feature,G):
			gcn_output = tf.TensorArray(
						dtype =tf.float32,
						size = self.timesteps*self.actual_batch,
						tensor_array_name = 'GCN_output')

			def train_step(time,gcn_output):
				train_timesteps = time//self.actual_batch
				train_batch = time%self.actual_batch
				g = G[train_timesteps, train_batch]
				x = o_feature[train_timesteps, train_batch]

				z_1 = tf.matmul(g,x)
				z_1 = tf.matmul(z_1,self.w1_re) #have not initialized to be continued
				z_1 = tf.nn.relu(z_1)

				z_2 = tf.matmul(g,z_1)
				z_2 = tf.matmul(z_2,self.w2_re)
				z_2 = tf.nn.relu(z_2)

				gcn_output = gcn_output.write(time,z_2)
				return(time+1,gcn_output)	

			time = tf.constant(0, dtype='int32', name='time')
			train_out = tf.while_loop(
				    cond = lambda time, *_: time < self.timesteps*self.actual_batch,
				    body = train_step,
				    loop_vars = (time, gcn_output),
				    parallel_iterations = 32,
				    swap_memory = True
				    )


			gcn_output = train_out[1]

			if hasattr(gcn_output,'stack'):
				output = gcn_output.stack()
			else:
				output = gcn_output.pack()
			'''
			if hasattr(gcn_output, 'stack'):
				output = gcn_output.stack()
			else:
				output = gcn_output.pack()
				 #[timestep * batch, obj_num,reduction_dim]
			'''
			
			output = tf.reshape(output, [self.timesteps,-1,self.obj_num,self.reduction_dim])
			output = tf.reduce_mean(output,2)#[timestep,batch,redu_dim]
			#rela_feature
			
      #encoder_result = GRU(output)
			return output

		relation_output = GCN(input_feature, G) # [timestep, batch, redu_dim]
		#relation_output = GRU(relation_output) # [timestep, batch, redu_dim]
		relation_output = tf.transpose(relation_output,perm=[1,0,2]) # [batch, timestep, redu_dim]

		return relation_output #to be continued
	...
	def encoder_allobjs(self, input_feature): ### for object feature
		...
		print('SamModel_VRel_GRU.py: encoder_allobjs: assignment:', assignment)
		axis = [1,0]+list(range(2,5)) 
	...
	def decoder_objV_att(self, initial_state, input_feature, relation_feature):
		...
		# fyt relation_feature [batch, timestep(encoder),redu_dim]
		...
		def step(x_t,h_tm1):
			...
			####for relation attention fyt
		
			ori_r_feature = tf.reshape(relation_feature,(-1,self.reduction_dim)) #[batch*timestep(encoder),redu_dim]
			attend_r_wx = tf.reshape(tf.nn.xw_plus_b(ori_r_feature, self.W_r_a, self.b_r_a),(-1,self.timesteps,self.attention_dim))#[batch, timestep(encoder), attention_dim]
			attend_r_uh_tm1 = tf.expand_dims(tf.matmul(h_tm1, self.U_r_a),dim=1) # shape [batch, 1, attention_dim]
			attend_r_e = tf.nn.tanh(tf.add(attend_r_wx,attend_r_uh_tm1)) # shape [batch, timestep(encoder), attention_dim]

			attend_r_e = tf.matmul(tf.reshape(attend_r_e,(-1,self.attention_dim)),self.W_r) # shape [batch*timestep(encoder)]
			attend_r_e = tf.nn.softmax(tf.reshape(attend_r_e,(-1,self.timesteps,1)),dim=1) # shape [batch, timestep(encoder), 1]
			attend_r_e = tf.reshape(attend_r_e,(-1,self.timesteps,1)) # shape [batch, timestep(encoder), 1]

			attend_r_fea = tf.multiply(tf.reshape(relation_feature,(-1,self.timesteps,self.reduction_dim)) , attend_r_e) 
			#attend_r_fea: shape [batch, timestep(encoder), redu_dim]
			attend_r_fea = tf.reduce_sum(attend_r_fea,reduction_indices=1) # shape [batch, redu_dim]
			...
			#attend_fea = tf.add(attend_fea, tf.matmul(attend_r_fea, self.r_A)) # shape [batch, 3*output_dim]

			attend_fea = tf.add(tf.matmul(attend_fea,self.A),tf.matmul(attend_r_fea, self.r_A)) 
	...
	def beamSearchDecoder_objV_att(self, initial_state, input_feature, relation_feature):
		...
		def step(x_t,h_tm1):
			...
			#print('SamModel_VRel_GRU.py: beamSearchDecoder: step: input_feature:', input_feature, 'o_input_feature:', o_input_feature)
			...
			#fyt
			this_relation_feature = tf.tile(relation_feature, [self.beam_size,1,1])
			...
			#########for relation attention fyt
			ori_r_feature = tf.reshape(this_relation_feature,(-1,self.reduction_dim)) #[batch*timestep(encoder),redu_dim]
			attend_r_wx = tf.reshape(tf.nn.xw_plus_b(ori_r_feature, self.W_r_a, self.b_r_a),(-1,self.timesteps,self.attention_dim))#[batch, timestep(encoder), attention_dim]
			attend_r_uh_tm1 = tf.expand_dims(tf.matmul(h_tm1, self.U_r_a),dim=1) # shape [batch, 1, attention_dim]
			attend_r_e = tf.nn.tanh(tf.add(attend_r_wx,attend_r_uh_tm1)) # shape [batch, timestep(encoder), attention_dim]
			...
			attend_r_fea = tf.multiply(tf.reshape(this_relation_feature,(-1,self.timesteps,self.reduction_dim)) , attend_r_e) 
			#attend_r_fea: shape [batch, timestep(encoder), redu_dim]
			attend_r_fea = tf.reduce_sum(attend_r_fea,reduction_indices=1) # shape [batch, redu_dim]
			...
			#attend_fea = tf.add(attend_fea, tf.matmul(attend_r_fea, self.r_A))
			attend_fea = tf.add(tf.matmul(attend_fea,self.A),tf.matmul(attend_r_fea, self.r_A))
		...
		def take_step_zero(x_0, h_0):
			...
			print('SamModel_VRel_GRU.py: beamSearchDecoder: take_step_zero: past_logprobs:', past_logprobs.get_shape().as_list(),
		...
		def test_step(time, x_t, h_tm1, past_symbols, past_logprobs, finished_beams, logprobs_finished_beams):
			...
			print('SamModel_VRel_GRU.py: beamSearchDecoder: test_step: beam_past_symbols:', beam_past_symbols.get_shape().as_list())
			...
			print('SamModel_VRel_GRU.py: beamSearchDecoder: test_step: past_symbols:', past_symbols.get_shape().as_list())
			...
			#print('SamModel_VRel_GRU.py: beamSearchDecoder: test_step: cond1:', cond1.get_shape().as_list(), cond1)

			for_finished_logprobs = tf.where(cond1,past_logprobs,tf.ones_like(past_logprobs,tf.float32)* -1e5) # shape [1, beam_size]
			#print('SamModel_VRel_GRU.py: beamSearchDecoder: test_step: for_finished_logprobs:', for_finished_logprobs)
			...
			#print('SamModel_VRel_GRU.py: beamSearchDecoder: test_step: done_indice_max:', done_indice_max, 'logprobs_done_max:', logprobs_done_max)
			...
			#print('SamModel_VRel_GRU.py: beamSearchDecoder: test_step: cond2:',cond2, 'cond3:',cond3, 'cond4:',cond4)
			...
			print('SamModel_VRel_GRU.py: beamSearchDecoder: test_step: symbols:',symbols, 'h:',h)
			print('SamModel_VRel_GRU.py: beamSearchDecoder: test_step: past_symbols:',past_symbols, 'past_logprobs:',past_logprobs)
			print('SamModel_VRel_GRU.py: beamSearchDecoder: test_step: finished_beams:',finished_beams, 
		...
		print('SamModel_VRel_GRU.py: beamSearchDecoder: after take_step_zero, symbols:',symbols,
	...
	def decoder_onestep(self):
		...
		relation_feature = self.r_encoder_output
	...
	def build_model(self):
		...
		#print('SamModel_VRel_GRU.py: build_model: input_feature:', input_feature.get_shape().as_list())
		#encoder_output = self.encoder_allobjs(input_feature) #shape [1+obj_num, batch, timestep, redu_dim*centers_num]


		#fyt v_encoder_output global frames  
		#o_encoder_output object regions
		...
		#fyt
		#input:reshaped_obj_feature shape[obj_num, batch, timestep, h, w, c]  
		#timestep-40 h-7 w-7 c-2048


		#output:relation_encoder_output shape[batch, timestep, redu_dim] 
		
		relation_encoder_output = self.encoder_relation(self.obj_feature_no_align,self.relation_feature)
		...
		if self.output_dim!=self.feature_dim:
			...
			last_output = v_last_output
		
		#print('SamModel_VRel_GRU.py: build_model: after encoder, last_output:', last_output.get_shape().as_list())
		#print('SamModel_VRel_GRU.py: build_model: v_encoder_output:', v_encoder_output.get_shape().as_list(),
		#	'o_encoder_output (VLAD):', o_encoder_output.get_shape().as_list())

		#fyt
		predict_score, predict_words , loss_mask= self.decoder_objV_att(last_output, v_encoder_output, relation_encoder_output)
		#predict_score: shape [batch, caplen, voc_size], scores for train_hidden_states
		#predict_words: shape [batch, caplen], for test hidden_states
		#loss_mask: shape [batch, caplen], float32, the float version of mask (bool)
		print('SamModel_VRel_GRU.py: build_model: after decoder, predict_score:', predict_score.get_shape().as_list(),
		...
		finished_beam, logprobs_finished_beams, past_symbols = self.beamSearchDecoder_objV_att(last_output, v_encoder_output, relation_encoder_output)
		print('SamModel_VRel_GRU.py: build_model: after beamSearchDecoder, finished_beam:', finished_beam.get_shape().as_list(),
	...
	def build_model_test(self):
		...
		return (last_output, v_encoder_output, relation_encoder_output), (dec_h, dec_logprobs)
