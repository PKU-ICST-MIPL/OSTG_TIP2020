...
from utils import SeqVladDataUtil_relation_global_correct as SeqVladDataUtil_R
#from model import SamModel 
from model import f_SamModel_ObjectV_att_NoShare_test as SamModel
from model import f_SamModel_R as SamModel_R
...
def evaluate_mode_by_shell(res_path,js):
	...
	command ='caption_eval/call_python_caption_eval.sh '+'\"'+os.path.abspath(res_path) +'\"'+ ' 2>&1 | tee -a ' + args.log_file
	os.system(command)
...
def main(hf,obj_hfs,f_type,
		...
		file=None, obj_file=None, obj_file_rev=None, rel_file=None, saveprefix='youtube',beam_size=5, obj_num=5):
	...
	with sess3.as_default():
		with g3.as_default():
			input_video3 = tf.placeholder(tf.float32, shape=(None,)+feature_shape,name='input_video')
			#input_object3 = tf.placeholder(tf.float32, shape=(None, obj_num)+feature_shape,name='input_object')
			input_captions3 = tf.placeholder(tf.int32, shape=(None,capl), name='input_captions')	
			input_relation3 = tf.placeholder(tf.float32, shape=(None,timesteps_v,obj_num,obj_num),name='input_relation')
			input_object_no_align3 =  tf.placeholder(tf.float32, shape=(None,obj_num)+feature_shape,name='input_video_no_align')
			actual_batch3 = tf.placeholder(tf.int32, shape=(None),name='actual_batch')

			dec_x_t_3 = tf.placeholder(tf.int32, shape=(None,None), name='dec_x_t')
			dec_h_tm1_3 = tf.placeholder(tf.float32, shape=(None, output_dim), name='dec_h_tm1')
			dec_v_in_feature3 = tf.placeholder(tf.float32, shape=(None, timesteps_v, reduction_dim*centers_num), name='dec_v_in_feature')
			dec_r_in_feature3 = tf.placeholder(tf.float32, shape=(None, timesteps_v, reduction_dim), name='dec_r_in_feature')
			...
			captionModel3 = SamModel_R.SoftModel(input_object_no_align3, actual_batch3, input_relation3, input_video3, input_captions3, dec_x_t_3, dec_h_tm1_3, dec_v_in_feature3, dec_r_in_feature3,
									voc_size, d_w2v, output_dim,
									reduction_dim=reduction_dim,
									centers_num=centers_num, 
									done_token=v2i['EOS'], max_len = capl, beamsearch_batchsize = 1, beam_size=5)
	...
	def perform_fusion(w1, w2, w3):
		...
		for batch_idx in xrange(num_batch):
			...
			_, data_r, data_obj_no_align = SeqVladDataUtil_R.getBatchVideoObjectFeature2(batch_caption,hf,obj_hfs,obj_num,feature_shape,rel_file)#fyt
			...
			out_enc_g3 = sess3.run(out_enc3, feed_dict={input_video3:data_v, input_relation3: data_r, input_object_no_align3: data_obj_no_align, actual_batch3: actual_batch_size})
			...
			h_0_3 = np.reshape(np.tile(h_0_3, [1, beam_size, 1]), (beam_size, output_dim))
			...
			for time in range(capl):
				...
				#fyt
				out_dec_g3 = sess3.run(out_dec3, feed_dict={dec_x_t_3:x_t, dec_h_tm1_3:h_tm1_3,
						dec_v_in_feature3:v_enc_out3, dec_r_in_feature3: r_enc_out3})
		...
		res_path = 'fusion/RF2_Redu_E6_res/%s_w1(%.2f)_w2(%.2f)_w3(%.2f).json' % (saveprefix, w1, w2, w3)
...
if __name__ == '__main__':
	...
	args.log_file = 'fusion/log_RF2_Redu_E6'
	print(args)
	...
	batch_size = 32
	saveprefix = 'objSim_NMS0.3_WV_att_NoShare_capl15s3_FwRevE3E8RelE6_right'
	...
	main(hf,obj_hfs,f_type, 
		...
		file='./data/msvd', obj_file=obj_file, obj_file_rev=obj_file_rev, rel_file=rel_file, saveprefix=saveprefix, beam_size=5, obj_num=obj_num)
