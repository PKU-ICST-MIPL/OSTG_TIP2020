...
def getBatchVideoObjectFeature2(batch_caption, hf, obj_hfs, obj_num, feature_shape, rel_file):
	#print('getBatchVideoObjectFeature2...')
	batch_size = len(batch_caption)

	#numpy.ndarray
	#(batch_size, timesteps_v, video_feature_dims, height, weight)
	input_video = np.zeros((batch_size,)+tuple(feature_shape),dtype='float32')
	...
	input_relation = np.zeros((batch_size,timesteps,obj_num,obj_num),dtype = 'float32')
	#relation_feature = np.zeros((timesteps*obj_num,obj_num),dtype = 'float32')

	
	obj_num_0 = len(obj_hfs)

	#input_object = np.zeros((batch_size,obj_num)+tuple(feature_shape),dtype='float32')#(batch_size, obj_num, timesteps_v, video_feature_dims, height, weight)
	obj_feature = np.zeros((obj_num_0, )+tuple(feature_shape),dtype='float32') #(obj_num,timesteps_v, video_feature_dims, height, weight)
	
	#fyt
	obj_feature_no_align = np.zeros((obj_num_0, )+tuple(feature_shape),dtype='float32') #(obj_num,timesteps_v, video_feature_dims, height, weight)
	input_object_no_align = np.zeros((batch_size,obj_num)+tuple(feature_shape),dtype='float32')#(batch_size, obj_num, timesteps_v, video_feature_dims, height, weight)

	#relation_path = '/home/fengyutian/data/msvd/similarity/'
	#relation_path = '/home/fengyutian/data/msvd/similarity_s_c_i/'
	#relation_path = rel_file
	for idx, caption in enumerate(batch_caption): #idx-0 caption-{'vid1':captions}
		
		for k,v in caption.items():# k-vid3 v-captions
			### read obj features
			for obj_i in range(obj_num_0):
			#for obj_i in range(1):
				feature = obj_hfs[obj_i][k]  #fyt how to use obj_hfs
				obj_feature[obj_i] = np.reshape(feature,feature_shape)
				#fyt
				obj_feature_no_align[obj_i] = obj_feature[obj_i]
			#obj_sim_file = obj_file+ '/' + k + '_similarity.txt'
			#obj_sim = np.loadtxt(obj_sim_file, dtype='int32')
			...
			#fyt  to be continued
			relation_path = rel_file + k +'_similarity.txt'
			relation_feature = np.loadtxt(relation_path,dtype='float32')  #[timesteps*obj,obj]
			relation_feature = np.reshape(relation_feature,[timesteps,obj_num,obj_num])
		
		input_relation[idx] = relation_feature
		input_object_no_align[idx] = obj_feature_no_align[0:obj_num]
	...
	# input_relarion: [batch, timestep, obj_num, obj_num] 
	return input_video, input_relation, input_object_no_align
